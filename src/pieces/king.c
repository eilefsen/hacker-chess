...
KingMove validate_king_move(BOARD_T(b), Move m, enum Color c, bool should_print) {
	if (!validate_basic(m)) {
		KingMove out = {false, None_Castle};
		return out;
	}

	Piece king_p = b[m.from.y][m.from.x];
	if (!king_p.has_moved && (m.to.y == 7 || m.to.y == 0)) {
		Piece r_rook_p = b[m.from.y][7];
		bool r_empty = b[m.from.y][5].kind == None &&
					   b[m.from.y][6].kind == None;
		if (m.to.x == 6 && r_empty && r_rook_p.kind == Rook && !r_rook_p.has_moved) {
			KingMove out = {true, Short_Castle};
			return out;
		}

		Piece l_rook_p = b[m.from.y][0];
		bool l_empty = b[m.from.y][1].kind == None &&
					   b[m.from.y][2].kind == None &&
					   b[m.from.y][3].kind == None;
	...
	if (validate_takes(b, m.to, c) || b[m.to.y][m.to.x].kind == None) {
		if (should_print) {
			puts("King takes!");
		}
